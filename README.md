...
Тело хода: `{"player_id": 1, "card_id": 101}`
Логика ходов:
Проверяется очерёдность хода
Карта должна быть в руке игрока
Сила карты запрашивается у Card-сервиса
Сила прибавляется к счёту игрока
Ход переходит к сопернику
Когда руки пусты — игра завершается, определяется победитель
---
Примеры запросов через Gateway
```bash
# Регистрация
